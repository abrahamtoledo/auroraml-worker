...
class webDownload{
	...
	protected function Digest($webFile, $content_type){
		...
		while($tk = strtok(";")){
			$tk = trim($tk);
			if ( stripos($tk, "charset") !== false ){
				list($foo, $charset) = explode("=", $tk, 2);
				$charset = trim($charset);
				break;
			}
		}
		
		// if (strpos($c_type, "text") === 0 &&
			// $charset == "utf-8"){
			//$webFile->content = utf8_encode($webFile->content);
		// }
		// end charset fixup
	...
	protected function digestHTML($webFile){
		...
		foreach ($dom->find("*[src], *[href], *[action]") as $elem){
			$key = $elem->src ? "src" : ($elem->href ? "href" : "action");
			if (!preg_match('#^(\#|mailto:|javascript:)#is', $elem->$key)){
				$newUrl = $base->ComputeNewUrl($elem->$key);
				if ($newUrl)
