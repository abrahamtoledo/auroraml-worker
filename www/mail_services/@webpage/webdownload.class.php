...
//		return;
		...
		print "mb_detected_enc: " . mb_detect_encoding($webFile->content) . "\n";
		print "Charset: $charset\n";

		// Setup DOM and XPath
		$dom = new DOMDocument();
		@$dom->loadHTML("<?xml encoding='utf-8'?>" . $webFile->content);

//		@$dom->loadHTML(mb_convert_encoding($webFile->content, 'HTML-ENTITIES', 'UTF-8') );
//		$webFile->content = $dom->saveHTML();
//		return;

		$xp = new DOMXPath($dom);
		$xp->registerNamespace('php', 'http://php.net/xpath');
		$xp->registerPhpFunctions(); // Allow all PHP functions
		$l = function($v) {
			return "php:functionString('strtolower', $v)";
		};

		// Resolve Package name
		if ($webFile->root) {
			// Este es el archivo principal
			$this->ResolvePackName($xp);
		}

		// Forzar javascript o imagenes o cualquier otro tag en caso de que
		// sea necesario.
		$meta = $xp->evaluate("string(//meta[{$l('@name')}='auroraml:force_tags'])");
		$force_tags = explode(",", str_replace(" ", "", $meta));
		$this->remove_selectors = array_diff($this->remove_selectors, $force_tags);

		// Add charset and x-location
		$head = $xp->query("//head[1]");
		if ($head->length > 0){
			$head = $head->item(0);
		}else{
			$head = NULL;
		...
		if ($head !== NULL && $charset !== NULL &&
			$xp->evaluate("count(//meta[{$l('@http-equiv')}='content-type'])") == 0){
		...
		if ($head !== NULL){
			$meta = $dom->createElement("meta");
			$meta->setAttribute("name", "x-location");
			$meta->setAttribute("content", $webFile->url);
			$head->appendChild($meta);
		}

		$body = $xp->query("//body[1]")->item(0);
		// Top Message
		if ($webFile->root){
...
HTML;
				...
				$elem = self::_createTreeElement($dom, $messageTag);
		...
		$base = $webFile->url;
		$baseTag = $xp->query("//head//base[1]");
		if ($baseTag->length > 0){
			$baseTag = $baseTag->item(0);
			$base = $base->ComputeNewUrl($baseTag->getAttribute("href"));
			$baseTag->parentNode
				->removeChild($baseTag);
		}
		...
		if (count($this->remove_selectors)){
			$q = "//" . implode("|//", $this->remove_selectors);
			foreach($xp->query($q) as $elem) {
				$elem->parentNode->removeChild($elem);
			}
		}

		// Remove refresh meta tag
		foreach ($xp->query("//meta[{$l('@http-equiv')}='refresh']") as $elem){
			$elem->parentNode->removeChild($elem);
		}

		// Show noscript blocks when javascript disabled
		if (in_array("script", $this->remove_selectors)){
			foreach($xp->query("//noscript") as $elem){
				self::_changeTagName($elem, "div");
			}
		}

		// (TED) Transforms and Extract Dependencies
		// 1) Normalize all Urls
		foreach ($xp->query("//*[@src or @href or @action]") as $elem){
			...
			$oldUrl = $elem->getAttribute($key);

			if (!preg_match('#^(\#|mailto:|javascript:)#is', $oldUrl)){
				if ($key == "action" && (is_bool($oldUrl) || !$oldUrl)){
					$newUrl = $webFile->url;
				}else{
					$newUrl = $base->ComputeNewUrl($oldUrl);
				}

				if ($newUrl){
					$elem->setAttribute($key, $newUrl->Save());
				}
			}
		}

		// Patch, ALL FORMS WITHOUT ACTION attribute
		foreach ($xp->query("//form[not(@action)]") as $elem){
			$elem->setAttribute('action', $webFile->url);
		}
		// End Patch
		...
		$q = "//*[@src] | //link[{$l('@rel')}='stylesheet' or {$l('@rel')}='shortcut icon' ]";
		if ($webFile->depth > 0) $q .= " | //a[@href]";

		foreach ($xp->query($q) as $elem) {
			// Download only page links with matching urls
			if (strtolower($elem->tagName) == "a" && $this->match_only != null &&
				!preg_match("#{$this->match_only}#is", $elem->getAttribute('href'))){
				continue;
			}

			$key = $elem->hasAttribute('src') ? 'src' : 'href';
			$attrVal = $elem->getAttribute($key);

			if (($url = Url::Parse($attrVal)) !== false){
				if ($tWebFile = $this->getMapFile($url)){ // File Already Downloaded
					$elem->setAttribute($key,
						$this->urlencode_keep_slashes(
							$this->getRelativePath($webFile->name, $tWebFile->name)
						)
					);
				}else{
					$depth = strtolower($elem->tagName) == "a" ? $webFile->depth - 1 : $webFile->depth;
		...
		foreach ($xp->query("//style") as $elem) {
			...
			$elem->nodeValue = preg_replace_callback($regexp,
				array($callback, "Callback"), $elem->nodeValue);
		}
		...
		print "Url: {$webFile->url}\n";
		print "DOM Encoding: {$dom->encoding}\n";
		print "DOM Actual Encoding: {$dom->actualEncoding}\n";
		print "DOM XML Encoding: {$dom->xmlEncoding}\n";
		$webFile->content = $dom->saveHTML();
		print "Detected output encoding: " . mb_detect_encoding($webFile->content) . "\n";

//		$webFile->content = mb_convert_encoding($dom->saveHTML(), 'iso-8859-1', "ASCII");
//		print mb_detect_encoding($dom->saveHTML());
		unset ($dom);
	}
	...
	private static function _createTreeElement(DOMDocument $dom, string $messageTag)
	{
		$dummy = new DOMDocument();
		$dummy->loadHTML("<div>{$messageTag}</div>");

		$elem = $dummy->getElementsByTagName('div')->item(0);
		$cloned = $dom->importNode($elem, true);
		return $cloned;
	}
	private static function _changeTagName(DOMElement $node, $name) {
