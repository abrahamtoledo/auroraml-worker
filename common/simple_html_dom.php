 ...
 * simple_html_dom_node::PrependChild($node)

 * simple_html_dom::create_element(...)
 * simple_html_dom::create_self_closed_element(...)
...
class simple_html_dom {
    ...
    function create_element($tag, $content = array(), $attributes = array()){
        $elem = new simple_html_dom_node($this);
        $elem->nodetype = HDOM_TYPE_ELEMENT;
        $elem->_[HDOM_INFO_BEGIN] = count($this->nodes) - 1;
        $elem->tag = $tag;
        ...
        if (is_string($content)){
            ...
            $elem->AddChild($content);
        }

        $elem->_[HDOM_INFO_ENDSPACE] = '';
        $elem->_[HDOM_INFO_END] = TRUE;

        return $elem;
    }

    function create_self_closed_element($tag, $attributes = array()){
        $elem = new simple_html_dom_node($this);
        $elem->nodetype = HDOM_TYPE_ELEMENT;
        $elem->_[HDOM_INFO_BEGIN] = count($this->nodes) - 1;
        $elem->tag = $tag;
